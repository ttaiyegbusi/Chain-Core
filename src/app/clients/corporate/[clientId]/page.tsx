...
export default function CorporateClientDetailPage({
  params,
}: {
  params: Promise<{ clientId: string }>;
}) {
  const { clientId } = use(params);
  const client = findClient("Corporate", clientId);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="ml-[316px]">
        <GlobalHeader title="Corporate Clients" />
        <section className="px-10 pb-10 pt-6">
          ...
          <div className="mt-2 text-xs text-text-muted">
            ...
            <Link href="/clients/corporate">Corporate</Link>
            <span className="mx-1.5">/</span>
          ...
          <div className="mt-10 rounded-lg border border-dashed border-border bg-surface-muted/30 p-12 text-center">
            ...
            <p className="mt-2 text-sm text-text-secondary">
              Corporate client detail page is not yet built.
            </p>
          </div>
        </section>
      </main>
